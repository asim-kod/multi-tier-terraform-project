module "app2_asg" {
  ...
  # Autoscaling group
  name            = "${local.environment}-app2-asg"
  use_name_prefix = false
  instance_name   = "${local.environment}-app2"
  ...
  traffic_source_attachments = {
    ex-alb = {
      traffic_source_identifier = module.alb.target_groups["app2"].arn
  ...
  # Launch template
  launch_template_name        = "${local.environment}-app2-lt"
  ...
  user_data = base64encode(templatefile("app2-install.sh", {
    db_username = var.db_username,
    db_password = var.db_password,
    db_endpoint = module.rdsdb.db_instance_address,
    db_name     = var.db_name
  }))
    ...
    request-count-per-target = {
      ...
      target_tracking_configuration = {
        predefined_metric_specification = {
          ...
          resource_label         = "${module.alb.arn_suffix}/${module.alb.target_groups["app2"].arn_suffix}"
        }
        target_value = 10
      }
    }
